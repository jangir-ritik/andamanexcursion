...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (!page || page.publishingSettings?.status !== "published") {
    return {
      title: "Privacy Policy | Andaman Excursion",
      description: "Read the privacy policy for Andaman Excursion services.",
    };
  }

  // Use SEO plugin data (meta field) or fallback to page data
  const seoTitle = page.meta?.title || page.title || "Privacy Policy | Andaman Excursion";
  const seoDescription =
    page.meta?.description || "Read the privacy policy for Andaman Excursion services.";
