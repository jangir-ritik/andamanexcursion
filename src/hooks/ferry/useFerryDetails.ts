...
export const useFerryDetails = (ferryId?: string, classId?: string): UseFerryDetailsReturn => {
  ...
  useEffect(() => {
    try {
      ...
      if (!currentFerry && searchResults && searchResults.length > 0) {
        currentFerry = searchResults.find((f: UnifiedFerryResult) => f.id === actualFerryId) || null;
      }
      ...
      if (currentFerry) {
        ...
        if (actualClassId && currentFerry.classes.length > 0) {
          const targetClass = currentFerry.classes.find((c) => c.id === actualClassId);
          if (targetClass) {
  ...
  }, [actualFerryId, actualClassId, selectedFerry, searchResults, selectClass, isLoading]);
